...
class VolumeRenderer(torch.nn.Module):
    ...
    def forward(self, radiance_field, depth_values, ray_directions):
        ...
        Returns
        -------
            rgb_map: [B, num_rays, 3]
                Estimated RGB color of a ray.
            disp_map: [B, num_rays]
                Disparity map. 1 / depth.
            weights: [B, num_rays, num_samples]
                Weights assigned to each sampled color.
            acc_map: [B, num_rays]
                Accumulated opacity along each ray.
            depth_map: [B, num_rays]
        ...
        ret = {
            'rgb_map': rgb_map, # torch.Size([batch_size, 3])
            'depth_map': depth_map, # torch.Size([batch_size])
            'weights': weights, # torch.Size([batch_size, num_samples])
            'mask_weights': mask_weights, # torch.Size([batch_size, num_samples])
            'acc_map': acc_map, # torch.Size([batch_size])
            'disp_map': disp_map # torch.Size([batch_size])
        }
        return ret
